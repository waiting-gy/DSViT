...
class ViT(nn.Module):
    def __init__(self, *, image_size, image_patch_size, frames, frame_patch_size, num_classes, dim, depth, heads, mlp_dim, pool = 'cls', channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0.):
        ...
        self.k_embedding = nn.Linear(1, dim)
        self.k_decode = nn.Linear(dim, 1)

        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches + 1 + 8, dim))
    ...
    def forward(self, video, k1, k2, k3, k4, k5, k6, k7, k8):
        ...
        x = torch.cat((cls_tokens, x, k), dim=1)

        x += self.pos_embedding[:, :(n + 1 + 8)]
        x = self.dropout(x)
